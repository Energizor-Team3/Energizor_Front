$(document).ready(function () {
  ...
  $('#editReservationButton').click(function () {
    ...
    if (isRadioChecked) {
      // 상단 내 예약내역 텍스트에 자원예약수정 추가
      $('.subject strong').after('<strong id="resourceReservationText">자원예약수정</strong>');

      // 내 예약내역 텍스트의 글자색을 검정색으로 변경
      $('.subject strong').css('color', 'black');
      ...
      // 자원예약수정 텍스트의 글자색을 #415CBE로 변경
      $('#resourceReservationText').css('color', '#415CBE');
    }
  });


  // 자원예약수정 텍스트 클릭 시
  $('body').on('click', '#resourceReservationText', function () {
    // 수정 화면 보이기 및 테이블, 버튼, select, 라벨 숨기기
    $('#reservationEditContainer').show();
    $('.content table, .content .action-buttons, .content select, .page_number_choice_text').hide();
  });
  ...
  $('#saveReservationButton').click(function () {
    ...
    $('#resourceReservationText').remove(); // 자원예약수정 텍스트 제거
  ...
  $('#cancelEditButton').click(function () {
    // 상단 내 예약내역 텍스트를 원래대로 돌리기
    $('#resourceReservationText').remove(); // 자원예약수정 텍스트 제거
    $('.subject strong').html('내 예약내역').css('color', '#415CBE'); // 내 예약내역 텍스트의 색상 원래대로 변경
